...
dash.register_page(__name__, path="/monthly-airport", name="Monthly Airport Data")


PROCESSED_DATA_FILE = os.path.join(os.getcwd(), "assets", "monthly_airport_data.json")
...
layout = html.Div([
    ...
    dcc.Store(id='monthly-airport-stored-data'),
    html.H2("📆 Monthly Breakdown"),

    dcc.Upload(
        id="upload-airport-data-monthly",
        children=html.Button("Upload CSV", className="btn btn-primary"),
        multiple=False,
    ),

    html.Div(id="file-name-airport-monthly", style={"margin": "10px 0"}),

    dcc.Tabs([
        dcc.Tab(label='📊 Breakdown', children=[
            dcc.Loading(
                id="loading-airport-breakdown-monthly",
                type="default",
                children=[
                    html.Div(id="summary-cards-airport-monthly", className="summary-cards"),
                    html.Div(id="monthly-airport-graph-container"),
                    html.Div(id="monthly-airport-table-container"),
                ],
            )
        ]),
        dcc.Tab(label='💡 LTV Analysis', children=[
            dcc.Loading(
                id="loading-airport-ltv-monthly",
                type="default",
                children=[
                    html.Div(id="ltv-airport-content-monthly")
                ]
            )
        ])
    ])
...
def clean_and_merge_data(contents_list):
    ...
    df_cleaned["job_date"] = pd.to_datetime(
            df_cleaned["job_date"].str.strip(),
            format="%d/%m/%y %H:%M:%S",
            errors='coerce',
            dayfirst=True
        ).dt.date
...
@dash.callback(
    [Output('monthly-airport-stored-data', 'data'),
     Output('file-name-airport-monthly', 'children', allow_duplicate=True),
     Output('monthly-airport-graph-container', 'children'),
     Output('monthly-airport-table-container', 'children'),
     Output('ltv-airport-content-monthly', 'children')],
    [Input('upload-airport-data-monthly', 'contents'),
     Input('url', 'pathname')],
    [State('monthly-airport-stored-data', 'data')],
...
def unified_callback(contents, pathname, stored_data):
    ...
    if trigger == 'upload-airport-data-monthly' and contents:
        try:
